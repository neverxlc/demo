...
/**
 * https://github.com/alibaba/DataX/blob/master/common/src/main/java/com/alibaba/datax/common/util/RetryUtil.java
 */
public final class RetryUtil {
     ...
     * @param sleepTimeMilliSecond 运动失败后休眠对应时间再重试
    ...
    public static <T> T executeWithRetry(Callable<T> callable,
                                         int retryTimes,
                                         long sleepTimeMilliSecond,
                                         boolean exponential) throws Exception{

        Retry retry = new Retry();
        return retry.doRetry(callable, retryTimes, sleepTimeMilliSecond, exponential, null);
    }
     ...
     * @param callable               实际逻辑
     * @param retryTimes             最大重试次数(>1)
     * @param sleepTimeMilliSecond   运行失败后休眠对应时间再重试
     * @param exponential            休眠时间是否指数递增
    ...
    public static <T> T executeWithRetry(Callable<T> callable,
        ...
        Retry retry = new Retry();
        return retry.doRetry(callable, retryTimes,sleepTimeMilliSecond, exponential, retryExceptionClass);
    }
    ...
    /**
     * 创建异步执行的线程池。特性如下：
     * core大小为0，初始状态无线程，无初始消耗。
     * max大小为5,最多五个线程
     * 60秒超时时间，闲置超过60秒线程会被回收。
     * 使用SynchronousQueue,任务不会排队，必须要有可用线程才能提交成功，否则会
...
//                    e.printStackTrace();
                    saveException = e;
                    if (i == 0) {
                        logger.error(String.format("Exception when calling callable,异常Msg:%s",
                    ...
                    if (i + 1 < retryTimes && sleepTimeInMilliSecond > 0) {
                        ...
                        logger.error(String.format("Exception when calling callable, 即将尝试执行第%s次重试，" +
                                "本次重试计划等待[%s]ms，实际等待[%s]ms,异常Msg:[%s]",i+1, timeToSleep, realTimeSleep,
                                e.getMessage()));
                    }
                }
            }
